#!/usr/bin/env bash
# Reusable launcher for ad-hoc vLLM endpoints used by the bench harness.
#
# Reads a profile env file from $LAB_PROFILE and starts a podman container
# named "homelab-lab-vllm" on the chosen GPUs. Stops anything already named
...
#   LAB_IMAGE           full image ref (e.g. docker.io/vllm/vllm-openai:latest)
#   LAB_GPUS            "all" | "0" | "1" | "0,1"
#   LAB_PORT            host port (e.g. 8010)
#   LAB_API_KEY         bearer token
#   LAB_MODEL           HF model ref (e.g. QuantTrio/Qwen3.6-35B-A3B-AWQ)
#   LAB_SERVED_NAME     served-model-name
#   LAB_VLLM_ARGS       extra vllm serve args (string)
#   LAB_ENV_VARS        space-separated KEY=VAL pairs to set inside the container
#
# Usage:
#   LAB_PROFILE=~/.config/homelab-control/lab-qwen36-35b-a3b-baseline.env \
#     bash scripts/launch_vllm_lab.sh up
#   bash scripts/launch_vllm_lab.sh down
